...
// [[Rcpp::export]]
Rcpp::List estimate(MatrixXd& x, MatrixXd& w, int n_itr = 40) {
  jointModel mo(x, w);
  quadrature quad(13);
  MatrixXd post_w;
  ...
  do{
    ...
    // std::cout << mo.getRho() << std::endl;
  	...
  	<< std::endl;
  	i++;
  } while(i < n_itr && abs(current_loglik - previous_loglik) / abs
  	(previous_loglik) > 1e-6);

  // std::cout << "Final observed Log-likelihood: " << computeLoglik(mo) <<
  // std::endl;
